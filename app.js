...
async function api(path, data) {
  if (PUBLIC) {
    ...
    if (path.startsWith('/api/diff?')) {
      ...
      return Object.fromEntries([...new Set([...Object.keys(before.files), ...Object.keys(after.files)])].map(name => [name, lineDiff(before.files[name] ?? "", after.files[name] ?? "", `v${before.id}/${name}`, `v${after.id}/${name}`)]));
    }
...
async function detail(id) {
  ...
  `<section class="panel"><div class="panel-head"><h2>Comparación de archivos</h2><span class="pill neutral">SOLO LECTURA</span></div><div class="detail-note">${esc(item.notes || 'Sin notas adicionales.')}</div><div class="form-panel"><div class="toolbar"><label>Comparar desde<select id="compare-from">${state.versions.filter(v => v.id !== item.id).map(v => `<option value="${v.id}" ${v.id === item.id-1 ? 'selected' : ''}>${ver(v.id)} · ${esc(v.title)}</option>`).join('') || '<option value="">Sin otra versión</option>'}</select></label><label>Archivo<select id="compare-file">${Object.keys(item.files).map(f=>`<option>${esc(f)}</option>`).join('')}</select></label><button id="show-snapshot">Ver archivo completo</button></div><p id="diff-description"></p></div><pre class="diff" id="diff-content"></pre></section><div class="callout">SHA-256: <code>${item.hash}</code><br>Origen: <code>${item.parent_hash || 'Versión inicial'}</code></div>`;
}
function fileControls() {
  const extra=Object.keys(draft.files).filter(f=>!['controller.py','parameters.json'].includes(f));
  return `<div class="editor-tabs">${['controller.py','parameters.json'].map(f=>`<button data-file="${f}" class="${f===activeFile?'active':''}">${f}</button>`).join('')}</div>${extra.length?`<label class="bundle-picker">Archivos de Webots y C<select id="bundle-file"><option value="">Seleccionar archivo…</option>${extra.map(f=>`<option value="${esc(f)}" ${f===activeFile?'selected':''}>${esc(f)}</option>`).join('')}</select></label>`:''}`;
}
async function refreshBundleHashes(files) {
  const name='webots_real/snapshot.json';
  if(!files[name]) return;
  const manifest=JSON.parse(files[name]);
  const entries=await Promise.all(Object.entries(files).filter(([n])=>n.startsWith('webots_real/')&&n!==name).map(async([n,content])=>{
    const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(content));
    return [n.slice('webots_real/'.length),Array.from(new Uint8Array(digest),b=>b.toString(16).padStart(2,'0')).join('')];
  }));
  const hashes=Object.fromEntries(entries);
  if(JSON.stringify(Object.entries(hashes).sort())!==JSON.stringify(Object.entries(manifest.sha256).sort())) {
    manifest.sha256=hashes;
    files[name]=JSON.stringify(manifest,null,2)+'\n';
  }
}
function realPanel() {
  const r=state.webots_real;
  if(!r) return '';
  return `<section class="panel form-panel"><div class="panel-head"><h2>Evaluación Webots del diseño de 495 g</h2><span class="pill neutral">SIN MEDIDAS FÍSICAS</span></div><p>Pololu #3063 · ruedas Ø42 mm · LiPo 2S · 5 ToF y 4 QTR. Física registrada en <a href="#versions/${r.revision_id}">${ver(r.revision_id)}</a>. ${r.rounds} asaltos: mismas condiciones para todos los algoritmos, con valores nominales y parámetros aleatorios.</p><p>${esc(r.conclusion)}</p>${PUBLIC ? `<div class="actions"><a class="button" href="https://github.com/ByEmmanuel/Sumo_Page/blob/main/INFORME_WEBOTS_REAL.md">Informe completo</a><a class="button" href="${asset('laboratorio-completo.zip')}" download>Descargar proyecto y evidencias</a><a class="button" href="${asset('calibracion_webots.json')}" download>Datos de calibración</a></div>` : ''}<div class="table-wrap"><table><thead><tr><th>ALGORITMO</th><th>COLOCACIÓN</th><th>FÍSICA</th><th>V / D / E</th><th>AUTO-SALIDAS</th></tr></thead><tbody>${r.groups.map(g=>`<tr><td>${esc(g.algorithm)}</td><td>${esc(g.mode)}</td><td>${g.randomized?'Aleatoria':'Nominal'}</td><td>${g.wins} / ${g.losses} / ${g.draws}</td><td>${g.self_outs}</td></tr>`).join('')}</tbody></table></div><p>150 asaltos por fila: 30 contra cada uno de cinco rivales. Estas frecuencias describen los escenarios ensayados; no predicen la probabilidad de ganar un torneo real. La bitácora conserva los cambios aceptados y los ensayos fallidos.</p><figure><img class="calibration-plot" src="${asset('real-calibration.png')}" alt="Curvas medidas en Webots: aceleración, frenada inversa, frenada a PWM cero y aproximación al borde."><figcaption>Verde: caso nominal. Gris: tres realizaciones aleatorias; incluye trayectorias que caen de la pista. Sus saltos no representan una frenada validada.</figcaption></figure></section>`;
}
function editor() {
  if (PUBLIC) return heading('Algoritmos del laboratorio.', 'Consulta la última versión publicada. Los cambios se registran en el laboratorio local.', exportButton) + `<section class="panel"><div class="panel-head"><h2>${ver(draft.base)} · ${esc(state.versions[0].title)}</h2><span class="pill neutral">SOLO LECTURA</span></div>${fileControls()}<textarea class="code-editor" readonly aria-label="Contenido del archivo" spellcheck="false">${esc(draft.files[activeFile])}</textarea></section><div class="callout">Esta es una copia pública del historial. Para registrar un cambio, abre el laboratorio local, guarda una nueva versión y vuelve a publicar. La simulación se ejecuta en Webots.</div>`;
...
function bind(page, id) {
  ...
  if(page === 'editor') {
    if($('#bundle-file')) $('#bundle-file').onchange=e=>{if(e.target.value){activeFile=e.target.value; $('#content').innerHTML=editor(); bind('editor');}};
